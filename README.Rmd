...
Notice that this statement (like the definition of the work before the panels), takes for granted that 1) AIs are powerful tools, and 2) universities can be leading players in their adoption. This attitude is almost fatalistic^[In the sense of "fated to"] in its enthusiasm, and perfectly consistent with the hype observed around AI this time^[The pattern of cyclical hypes around AI even has a name: [AI winters](https://en.wikipedia.org/wiki/AI_winter); the current one with chatbots in the pole position is just the most recent of several hype cycles [see @siegel_ai_2023]] [@angwin_press_2024]. 
...
AI would even write code, perhaps helping itself to some self-programming (AI self-help?) as an outcome.

Figure @\ref(fig:ai-writing-code)

```{r ai-writing-code, out.width="50%", fig.cap="\\label{fig:ai-bug-fix}Figure X. Example of an AI writing code"}
