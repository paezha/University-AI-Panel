...
```{r plot-simulated-data, fig.cap="Simple Example of a Regression Model"}
ggplot(df,
       aes(x = x,
           y = y)) +
  geom_point() +
...
Predict:
```{r data-vs-predictions-comparison, fig.cap="Training data and model output compared"}
...
ChatGPT consumes substantially more than a search...are the results better? [@https://www.brusselstimes.com/1042696/chatgpt-consumes-25-times-more-energy-than-google]

Compare the results:

![A search using Duck Duck Go](images/logical-puzzle-web-search.png)

![A conversation with ChatGPT-4](images/logical-puzzle-chat-gpt-4.png)
